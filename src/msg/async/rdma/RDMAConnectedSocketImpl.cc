  ...
  : RDMAConnMgr(cct, sock, ib, s, w, 2), con_handler(new C_handle_connection(this))
{
  ...
  int ret = RDMAConnMgr::create_queue_pair();
  assert(!ret);
...
RDMAConnectedSocketImpl::~RDMAConnectedSocketImpl()
{
  ...
  for (unsigned i=0; i < wc.size(); ++i) {
    ret = cmgr->ibdev->post_chunk(reinterpret_cast<Chunk*>(wc[i].wr_id), cmgr);
    assert(ret == 0);
  }
  for (unsigned i=0; i < buffers.size(); ++i) {
    ret = cmgr->ibdev->post_chunk(buffers[i], cmgr);
    assert(ret == 0);
  }
...
ssize_t RDMAConnectedSocketImpl::read(char* buf, size_t len)
{
  ...
  ldout(cct, 20) << __func__ << " notify_fd : " << i << " in " << *this << " r = " << r << dendl;
  if (error)
    return -error;
  ssize_t read = 0;
  if (!buffers.empty())
  ...
  ldout(cct, 20) << __func__ << " poll queue got " << cqe.size() << " responses. " << *this << dendl;

  Device *ibdev = cmgr->ibdev;
  ...
  for (size_t i = 0; i < cqe.size(); ++i) {
    ...
    ldout(cct, 0) << __func__ << " qp=" << cmgr->get_qp()->get_qp() << " read chunk=" << chunk << dendl;
...
ssize_t RDMAConnectedSocketImpl::send(bufferlist &bl, bool more)
{
    ...
    return 0;
  {
    ...
    if (!cmgr->connected) {
      ldout(cct, 20) << __func__ << " fake send to upper " << *this << dendl;
      return bytes;
    }
  }
  ldout(cct, 20) << __func__ << " " << *this << dendl;
...
{
  ldout(cct, 20) << __func__ << " " << *this << " " << tx_buffers[0] << dendl;
...
{
  ldout(cct, 0) << __func__ << " sending FIN " << *this << dendl;
...
{
  uint64_t i = 1;
  int retVal = write(notify_fd, &i, sizeof(i));
  if (retVal == -1) {
