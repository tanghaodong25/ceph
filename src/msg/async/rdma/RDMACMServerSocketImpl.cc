...
#undef dout_prefix
#define dout_prefix *_dout << " RDMACMServerSocketImpl "

RDMACMServerSocketImpl::RDMACMServerSocketImpl(CephContext *cct, Infiniband* i, RDMADispatcher *s, RDMAWorker *w, entity_addr_t& a) : RDMAServerSocketImpl(cct, i, s, w, a)
{
...
int RDMACMServerSocketImpl::listen(entity_addr_t &sa, const SocketOptions &opt)
{
...
int RDMACMServerSocketImpl::accept(ConnectedSocket *sock, const SocketOptions &opt, entity_addr_t *out, Worker *w)
{
  ...
  RDMACMConnectedSocketImpl* server = new RDMACMConnectedSocketImpl(cct, infiniband, dispatcher, dynamic_cast<RDMAWorker*>(w), &info);
