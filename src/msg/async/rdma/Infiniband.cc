...
Device::Device(CephContext *cct, ibv_device* d, struct ibv_context *dc): device(d), device_attr(new ibv_device_attr), active_port(nullptr)
{
  if (device == NULL) {
...
Infiniband::QueuePair::QueuePair(
    ...
    uint32_t tx_queue_len, uint32_t rx_queue_len, struct rdma_cm_id *cm_id_, uint32_t q_key)
...
int Infiniband::QueuePair::init()
{
  ...
  if (!cct->_conf->ms_async_rdma_cm) {
    ...
    if (rdma_create_qp(cm_id, pd, &qpia)) {
      lderr(cct) << __func__ << " failed to create queue pair with rdmacm library" << cpp_strerror(errno) << dendl;
      return -1;
    }
    qp = cm_id->qp;
  }
 ...
 */
Infiniband::QueuePair* Infiniband::create_queue_pair(CephContext *cct, CompletionQueue *tx, CompletionQueue* rx, ibv_qp_type type, struct rdma_cm_id *cm_id)
{
