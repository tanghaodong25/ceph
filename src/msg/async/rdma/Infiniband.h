...
class DeviceList {
  ...
  Device** devices;
 public:
  DeviceList(CephContext *cct): device_list(ibv_get_device_list(&num)), device_context_list(rdma_get_devices(&num)) {
...
class Infiniband {
  ...
  class QueuePair {
   public:
    QueuePair(CephContext *c, Infiniband& infiniband, ibv_qp_type type,
              ...
              uint32_t tx_queue_len, uint32_t max_recv_wr, struct rdma_cm_id *cm_id_, uint32_t q_key = 0);
    ~QueuePair();
 ...
 public:
  ...
  QueuePair* create_queue_pair(CephContext *c, CompletionQueue*, CompletionQueue*, ibv_qp_type type, struct rdma_cm_id *cm_id);
