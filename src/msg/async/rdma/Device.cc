...
void Device::init(int ibport, RDMAConnMgr* conn_mgr)
{
  ...
  if (!initialized) {
    ...
    if (support_srq) {
      ...
      max_recv_wr = std::min(device_attr->max_qp_wr/2, (int)cct->_conf->ms_async_rdma_recv_queue_per_connection);
    }
    ...
    memory_manager = new MemoryManager(this, pd,
				       cct->_conf->ms_async_rdma_enable_hugepage, cct);

    uint32_t recv_mem_size = cct->_conf->ms_async_rdma_receive_buffers;
    memory_manager->register_rx_tx(
	cct->_conf->ms_async_rdma_buffer_size, recv_mem_size, max_send_wr);
						 ...
						 RDMAConnMgr *cmgr)
{
  Infiniband::QueuePair *qp = new QueuePair(
      cct, *this, type, port, srq, tx_cq, rx_cq, max_send_wr, max_recv_wr, cmgr);
  if (qp->init()) {
    delete qp;
    return NULL;
  }
...
int Device::post_channel_cluster(RDMAConnMgr* conn_mgr)
{
  ...
  int r = 0;
  ldout(cct, 10) << __func__ << " before reserve rx: " << memory_manager->free_buffer_size() <<dendl;
  if (support_srq) {
  ...
  ldout(cct, 10) << __func__ << " after reserve rx: " << memory_manager->free_buffer_size() <<dendl;
  return 0;
}
