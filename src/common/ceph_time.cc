...
real_clock::time_point real_clock::now(const CephContext* cct) noexcept {
  ...
  // TODO: After we get the time-literal configuration patch in, just
  // add the configured duration.
  if (cct)
...
real_clock::time_point real_clock::from_timespec(
...
real_clock::time_point coarse_real_clock::from_timespec(
  const struct ceph_timespec& ts) {
  return time_point(seconds(ts.tv_sec) + nanoseconds(ts.tv_nsec));
}
} // namespace  time_detail
