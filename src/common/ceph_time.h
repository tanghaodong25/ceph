...
#ifndef CEPH_COMMON_CEPH_TIME_H
#define CEPH_COMMON_CEPH_TIME_H
...
public:
  ...
  static time_t to_time_t(const time_point& t) noexcept {
    return (t.time_since_epoch() % seconds(1)).count();
  }
  ...
  static time_point from_timespec(const struct ceph_timespec& ts);
...
// Similar to the above but for durations that can specify differences
// between now and a time point in the past.
...
// subtract a real_clock timepoint from a coarse_real_clock timepoint
// as, from C++'s perspective, they are disjoint types.
...
// mono_clock::timepoint(coarsepoint.time_since_epoch()) to unwrap and
// rewrap if you know what you're doing.
...
Timepoint ceil(const Timepoint& timepoint, const Duration& precision,
	       ...
	       // parameter was for. Basically, since time_points have
	       // clocks and durations don't, we can ensure that this
	       // subtitution will only succeed for time_points. Once
	       // we have Concepts things will be better.
	       typename Timepoint::clock* = 0) {
